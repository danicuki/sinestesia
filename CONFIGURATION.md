...
| `LYRICS_CHUNK_GEMINI_MODEL` | `gemini-3.6-flash` | — | Model asked to split a loaded song's full lyrics into visually coherent scene units (see Sinestesia.LyricsChunker), replacing the old fixed-word/fixed-line guess with a real per-song read of the whole text. Runs once per song, off the critical path — never blocks a render. |
