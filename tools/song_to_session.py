...
def build_events(words: list[dict], gap_s: float) -> list[dict]:
    """Reconstruct the live STT stream: words accumulate into an utterance,
    emitting a growing partial per word; the utterance commits (a `final`) at a
    phrase boundary, then a fresh one starts. Mimics what the realtime STT
    emits live, so the Director fires on the same growing fragments it sees on
    stage.

    Batch Scribe doesn't do VAD — consecutive word timings are tight even
